...
def main():
    path = 'H24PbC8_I2N_2.xyz' #Main input file containing replicatable unit cell.
    ...
    a, b, c, replication_factors, alpha, beta, gamma = 6.494, 29.461, 9.2666, (1, 1, 1), 90, 91.777, 90 #UNITCELL INPUTS FOR REPLICATION
    atoms= replicate(atoms, a, b, c, alpha, beta, gamma, replication_factors)
    #atoms = replicate(atoms, a=6.494, b=29.461, c=9.2666, replication_factors=(1, 1, 1), alpha=90, beta=91.777, gamma=90)

    # Butylammonium (BA) + Methylammonium (MA) unitcell
    #a, b, c, replication_factors, alpha, beta, gamma = 8.9470, 8.947, 20.1757, (1, 1, 1), 90, 90, 90
    #atoms = replicate(atoms, a, b, c, alpha, beta, gamma, replication_factors)
    ...
    # Bond list containing bonds within the original .xyz structure
    ...
    #Bonds present within molecules.xyz nescesary for alligning atom id's for angle and dihedral finding
    #This is done by simply running the molecules.xyz output data back into the bond calculation function
    #The unit cell info does need to be inputted once again.
    print("\nBOND CALCULATION FOR molecules.xyz\n")
    totatoms, atomsnew = read_atoms('molecules.xyz')
    atomsnew= replicate(atomsnew, a, b, c, alpha, beta, gamma, replication_factors)
    bonds = calcbonds(atomsnew)[1]
    print(f'Total bonds: {calcbonds(atoms)[0]}')
    print(bonds)
    with open('bondlistalligned.txt', 'w') as file:
        for bond in bonds:
            #print(bond)
            file.write(f'{bond}\n')
